...
handler.before = async function (m) {
  ...
  if (m.quoted.id == this.tebakgame[id][0].id) {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
      ...
      this.sendButton(m.chat, benar + ` +${this.tebakgame[id][2]} XP\n+1 Tiketcoin`, wm, 'Lagi?', '.tebakgame', m)
      clearTimeout(this.tebakgame[id][3])
      delete this.tebakgame[id]
    } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) this.sendButton(m.chat, dikit, wm, 'Bantuan', '.tega', m)
    else this.sendButton(m.chat, salah, wm, 'Bantuan', '.tega', m)
  }
  return !0
}
handler.exp = 0
