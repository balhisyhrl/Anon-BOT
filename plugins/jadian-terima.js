...
let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(user === m.sender) return conn.reply(m.chat, `Tidak bisa berhubungan dengan diri sendiri!`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `Tidak bisa berhubungan dengan saya t_t`, m)
