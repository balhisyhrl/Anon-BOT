...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebaklirik[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            this.sendButton(m.chat, benar + ` +${this.tebaklirik[id][2]} XP\n+1 Tiketcoin`, wm, 'Lagi?', '.tebaklirik', m)
            clearTimeout(this.tebaklirik[id][3])
            delete this.tebaklirik[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) this.sendButton(m.chat, dikit, wm, 'Bantuan', '.teli', m)
        else this.sendButton(m.chat, salah, wm, 'Bantuan', '.teli', m)
    }
    return !0
}
handler.exp = 0
