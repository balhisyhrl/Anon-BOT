...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.unsur.toLowerCase().trim()) {
            ...
            this.sendButton(m.chat, benar + ` +${this.tebakkimia[id][2]} XP\n+1 Tiketcoin`, wm, 'Lagi?', '.tebakkimia', m)
            clearTimeout(this.tebakkimia[id][3])
            delete this.tebakkimia[id]
        } else if (similarity(m.text.toLowerCase(), json.unsur.toLowerCase().trim()) >= threshold) this.sendButton(m.chat, dikit, wm, 'Bantuan', '.teki', m)
        else this.sendButton(m.chat, salah, wm, 'Bantuan', '.teki', m)
    }
    return !0
}
handler.exp = 0
