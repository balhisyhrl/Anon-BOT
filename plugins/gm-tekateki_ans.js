...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tekateki[id][0].id) {
        ...
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) this.sendButton(m.chat, dikit, wm, 'Bantuan', '.tekki', m)
        else this.sendButton(m.chat, salah, wm, 'Bantuan', '.tekki', m)
    }
    return !0
}
handler.exp = 0
